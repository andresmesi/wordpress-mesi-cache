...
/**
 * ==========================================================
 * Mesi Cache — Auto-regenerate cache on post/page publish
 * ----------------------------------------------------------
 * Cada vez que se publica o actualiza una entrada o página:
 *   - Genera el HTML estático de esa entrada/página
 *   - Elimina y regenera el index.html (home) al final del ciclo
 * ==========================================================
 */
add_action( 'save_post', function( $post_id, $post, $update ) {
    // Ignorar revisiones, autosaves o borradores
    ...
    // Evitar loops
    if ( defined( 'MESI_CACHE_REGEN' ) ) {
        return;
    }
    define( 'MESI_CACHE_REGEN', true );

    // Generar cache del post actual
    if ( function_exists( 'mesi_cache_generate_post' ) ) {
        mesi_cache_generate_post( $post_id );
    }

    // Regenerar home más tarde (cuando WP ya terminó el guardado)
    add_action( 'shutdown', function() {
        if ( function_exists( 'mesi_cache_generate_home' ) ) {

            // Eliminar index.html viejo antes de regenerar
            $home_file = mesi_cache_file_for_home();
            if ( file_exists( $home_file ) ) {
                wp_delete_file( $home_file ); // reemplaza unlink()
            }

            // Regenerar home
            mesi_cache_generate_home();
        }
    });
}, 10, 3 );
